...
var genModel = function(scheme, modelName){
  ...
  var newModelObject = function(partial){
    ...
    self.save = function(cb){
      ...
      console.log(util.inspect(scheme._server), util.inspect(scheme._server.db));
      scheme._server.db.collection(modelName);
      scheme._server.db[modelName].insert(self.values, 
          function(err,doc){
            cb(err, doc);
      });
    };
    ...
    self.embed = function(fieldName, object){
      if (scheme._derives[fieldName]){
        if (Array.isArray(scheme._derives[fieldName])){
          if (object._modelName && scheme._derives[fieldNAme][0] == object._modelName){
          ...
          if (object._modelName && scheme._derives[fieldNAme] == object._modelName){
        ...
        throw new Error("field name: '" + fieldName + "' not found in scheme!");
      }
    };
  };
