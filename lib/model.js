...
var Model = function(schema, modelName) {
  ...
  var Instance = function(partial, validate_all) {
    ...
    var genUpdateQueue = function(lookups, diff, globals) {
      var field, array$, query, command, containingFieldDef, i, j, updateQueue = [], propQueue = [];
      for (i = 0; i < lookups.length; i ++ ){
        if (lookups[i].field === null) {
          ...
          containingFieldDef = schema.remongo.models[lookups[i].model]
            .getFieldDef(lookups[i].field);
          if (containingFieldDef.include_anyway) {
            propQueue.push(lookups[i]);
          }
          query = {};
          ...
          for (j = 0; j < globals.length; j++) {
            command[lookups[i].field + array$ + '.' + globals[i]] =
              diff[globals[i]];
          }
          updateQueue.push({
            model: lookups[i].model,
    ...
    self.save = function(cb) {
      ...
      if (self._hasID) {
        diff = genDiff();
        console.log(findGlobals(diff));
        console.log(
          genUpdateQueue(schema.remongo.lookups[modelName], diff, findGlobals(diff))
        );
        for (i = 0; i < schema.remongo.lookups[modelName].length; i++) {
          lookup = schema.remongo.lookups[modelName][i];
        }
        schema.remongo.db[modelName].update(
          {_id: self.values._id},
          self.values,
          function(err, docs) {
            var warnfn = function(err) {
              if (err) {
                console.warn("propogated update failed for model:", 
                  model, "\nerror:", err);
              }
            };
            var update_array = [],path, def, i, query = {}, update_vals = {}, 
              model, field, embedder_schema, extra_query = {}, lookup_id, lookup;
            if (self.global_changes.length) {
              for (lookup_id in schema.remongo.lookups[modelName]) {
                for (i = 0; i < self.global_changes.length; i += 1) {
                  //we changed a public field, so we need to make sure that 
                  //changes propogate
                  lookup = schema.remongo.lookups[modelName][lookup_id];
                  embedder_schema = schema.remongo.models[lookup.model].schema;
                  def = embedder_schema.sub_schemas.embeds[lookup.field];
                  query[lookup.field + '._id'] = self.values._id;
                  if (def && def.array === true) {
                    update_vals[lookup.field + ".$." + self.global_changes[i]] = 
                  ...
                  schema.remongo.db[lookup.model].update(
                    query, 
                    {$set:update_vals}, 
                    {multi: true}, 
                    warnfn
                  );
                  // if the updated object exist as an "include_anyway" field 
                  // of a different schema, then we also need to make sure that
                  // an update is executed wherever an object of the host-
                  // schema is embedded.
                  query = {}, update_vals = {}, extra_query = {};
                }
      ...
      if (self._hasID) {
        ...
        var createRemoveFunc = function(lookup) {
          return function(callback) {
            ...
            console.log('i\'m here', lookup.model);
            embedder_schema = schema.remongo.models[lookup.model].schema;
